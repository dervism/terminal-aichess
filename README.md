...
Implemented null-move pruning in both ChessAI and ImprovedParallelChessAI:                                                                                                         
  - Added Bitboard.makeNullMove() — flips side to move without moving a piece                                                                                                        
  - Skips when: in check, PV node, depth < 3, or side has only pawns+king (zugzwang risk)                                                                                            
  - Adaptive reduction: R=2 normally, R=3 at depth >= 6                                                                                                                              
  - Doesn't trust mate scores from null-move (clamps to beta)                                                                                                                        
                                                         
  Implemented SEE (Static Exchange Evaluation):                                                                                                                                      
  - New SEE.java class — simulates the full capture exchange sequence on a square                                                                                                    
  - Uses least-valuable-attacker ordering, X-ray discovery through sliding pieces                                                                                                    
  - Move ordering: Captures now scored by SEE instead of MVV-LVA. Winning/equal captures ranked above killers, losing captures ranked below                                          
  - Quiescence pruning: Losing captures (SEE < 0) are skipped in quiescence search, reducing search tree size significantly     